...
public:
	...
	// Base core
	std::shared_ptr<Core> base_core;

	CoreMapper(std::shared_ptr<Core> c);

	CoreMapping genLayerMap(const Layer& layer, len_t batch_size, bool wgtB);
...
class PolarMapper: public CoreMapper{
	...
	public:
		Instance(std::shared_ptr<PolarCore> _core);
...
public:
	PolarMapper(std::shared_ptr<PolarCore> _core);
...
class EyerissMapper : public CoreMapper{
	class Instance{
		...
		std::shared_ptr<EyerissCore> core;

		void getCost(const ConvWl& wl);

	public:
		Instance(std::shared_ptr<EyerissCore> _core);

		void print_loop_nest(const ConvWl& wl, std::ostream& os = std::cout) const;
		CoreMapping genMapping(const ConvWl& wl);
	};
...
public:
	EyerissMapper(std::shared_ptr<EyerissCore> _core);
