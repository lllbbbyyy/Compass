...
bool ConvLayer::set_padded_ifm(const fmap_shape& padded_shape){
	...
	padded_ifm_shape.c = padded_shape.c;
...
// std::unique_ptr<Layer> GroupConvLayer::clone() const
// {
//     return std::make_unique<GroupConvLayer>(*this);
// }
...
// {
//     return std::make_unique<PoolingLayer>(*this);
// }
...
// std::unique_ptr<Layer> EltwiseLayer::clone() const
// {
//     return std::make_unique<EltwiseLayer>(*this);
// }
...
// std::unique_ptr<Layer> PTPLayer::clone() const
// {
//     return std::make_unique<PTPLayer>(*this);
// }
...
// {
//     return std::make_unique<TransposeLayer>(*this);
// }

void UpsampleLayer::Workload::init(){
	W = (W == 0)?H:W;
	sW = (sW == 0)?sH:sW;
	update_op();
	assert(tot_op>0 && H>0 && W>0 && K>0);
	assert(sK >= 1 && K % sK == 0);
	assert(sH >= 1 && H % sH == 0);
	assert(sW >= 1 && W % sW == 0);
}

void UpsampleLayer::Workload::update_op(){
	tot_op = K*H*W;
}

access_t UpsampleLayer::Workload::calc_op(len_t batch_size) const{
	return tot_op*batch_size;
}

UpsampleLayer::UpsampleLayer(const std::string& _name, const Workload& _wl):
	Layer(_name), wl(_wl){
	wl.init();
	ifm_shape = fmap_shape(wl.K/wl.sK, wl.H/wl.sH, wl.W/wl.sW);
	ofm_shape = fmap_shape(wl.K, wl.H, wl.W);
	padded_ifm_shape = ifm_shape;
	padded_ifm_shape.update_size();
	pad_h = pad_w = 0;
...
const UpsampleLayer::Workload& UpsampleLayer::get_workload() const{
	return wl;
}

const fmap_shape& UpsampleLayer::real_ifmap_shape() const{
	return padded_ifm_shape;
}

vol_t UpsampleLayer::weight_size() const{
	return 0;
}

access_t UpsampleLayer::get_num_op(len_t batch_size) const{
	return wl.calc_op(batch_size);
}

void UpsampleLayer::ofm_to_wgt(fmap_range& ofm_range) const{
	(void) ofm_range;
}

bool UpsampleLayer::fmap_channel_rel() const{
	return true;
}

// std::unique_ptr<Layer> UpsampleLayer::clone() const
// {
//     return std::make_unique<UpsampleLayer>(*this);
// }

bool UpsampleLayer::set_padded_ifm(const fmap_shape& padded_shape){
	return padded_shape == padded_ifm_shape;
}

void UpsampleLayer::ofm_to_ifm(fmap_range& ofm_range) const{
	ofm_range.c.from = ofm_range.c.from / wl.sK;
	ofm_range.h.from = ofm_range.h.from / wl.sH;
	ofm_range.w.from = ofm_range.w.from / wl.sW;
	ofm_range.c.to = DIVCEIL(ofm_range.c.to, wl.sK);
	ofm_range.h.to = DIVCEIL(ofm_range.h.to, wl.sH);
	ofm_range.w.to = DIVCEIL(ofm_range.w.to, wl.sW);
}