...
CoreMapper::CoreMapper(std::shared_ptr<Core> c):base_core(c){}
...
PolarMapper::PolarMapper(std::shared_ptr<PolarCore> _core)
...
CoreMapper::CoreMapping PolarMapper::genMapping(const ConvWl& wl)const{
...
EyerissMapper::EyerissMapper(std::shared_ptr<EyerissCore> _core)
	: CoreMapper(_core), core(_core) {}

CoreMapper::CoreMapping EyerissMapper::genMapping(const ConvWl& wl)const{
...
CoreMapper::CoreMapping CoreMapper::genLayerMap(const Layer& layer, len_t batch_size, bool wgtB){
	if(REF_IS_INSTANCE(layer, ConvLayer)){
		// Conv Layer...
		const ConvLayer& cl=static_cast<const ConvLayer&>(layer);
		ConvWl wl(cl.get_workload(), batch_size);
		...
		DEBUG(layer.get_name(), "is not supported");
		assert(false);
		return CoreMapping();
	}
}
...
PolarInst::Instance(std::shared_ptr<PolarCore> _core):core(_core){
	// Initialize
	hasWL2 = false;
	hasAL2 = false;
	hasWBSD = false;
...
EyerissInst::Instance(std::shared_ptr<EyerissCore> _core):core(_core){}
...
CoreMapper::CoreMapping EyerissInst::genMapping(const CoreMapper::ConvWl& wl) {
	...
	best_map *= wl.nGroup;
	// print_loop_nest(wl);
	// std::cout<<"wl.nGroup "<<wl.nGroup<<std::endl;
	return best_map;
}

void EyerissInst::print_loop_nest(const ConvWl& wl, std::ostream& os) const {
