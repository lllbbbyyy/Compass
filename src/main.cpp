...
int main()
{
	...
	{
		chips.emplace_back(createPolarCoreMapper(1024, 100 KB));
	...
	ga_engine.run();
	auto [ga_latency, ga_energy, ga_mc] = ga_engine.get_best_res();
	cout<<"GA result: latency: " << ga_latency << " energy: " << ga_energy << " mc: " << ga_mc << endl;
