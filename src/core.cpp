...
vol_t PolarCore::getSRAMSize() const{
	return ul3.Size + (al1.Size + wl1.Size + ol1.Size) * bus.aLen * bus.oLen;
}

bw_t PolarCore::getNoCBW() const{
	return bus.busBW;
}
...
vol_t EyerissCore::getSRAMSize() const{
	return ul2.Size + (al1.Size + wl1.Size + pl1.Size) * pes.Xarray * pes.Yarray;
}

bw_t EyerissCore::getNoCBW() const{
	return std::max(pbus.BusBW,std::max(ibus.BusBW,wbus.BusBW));
}
