...
# ======================= 强化学习环境 =======================
class ChipletEnv:
    def __init__(self, base_config, directory, homo_best_cost):
        self.base_config = base_config
        self.directory = directory
        self.log_id = 0
        self.num_chiplets = base_config["num_chiplets"]
        self.granularity = GRANULARITY  # 使用全局粒度设置
        self.homo_best_cost = homo_best_cost  # 同构最优成本用于归一化
    ...
    def step(self, action_idx):
        ...
        raw_cost = cost_func(latency, energy, mc)
        
        # 使用同构最优成本进行归一化
        normalized_cost = raw_cost / self.homo_best_cost
        ...
        return self.state, reward, raw_cost, normalized_cost
...
# ======================= 异构微调部分 (强化学习) =======================
def hetero_finetune_with_rl(base_config, homo_best_cost, directory, max_evals=100):
    env = ChipletEnv(base_config, directory, homo_best_cost)
    ...
    for eval_count in range(1, max_evals+1):
        ...
        # 执行动作
        next_state, reward, raw_cost, normalized_cost = env.step(action_idx)
        ...
        # 检查是否有改进 (使用原始成本)
        if raw_cost < best_raw_cost - 1e-4:  # 使用小阈值防止浮点误差
            improvement = best_raw_cost - raw_cost
            best_reward = reward
            best_raw_cost = raw_cost
            best_normalized_cost = normalized_cost
            best_config = copy.deepcopy(env.base_config)
            no_improve_count = 0
            pbar.set_postfix_str(f"改进: {improvement:.4f}, 归一化成本: {normalized_cost:.4f}, 原始成本: {raw_cost:.2f}, 连续无改进: 0/{patience}")
        else:
            no_improve_count += 1
            pbar.set_postfix_str(f"连续无改进: {no_improve_count}/{patience}, 归一化成本: {normalized_cost:.4f}, 原始成本: {raw_cost:.2f}")
    ...
    # 最终更新
    agent.finish_episode()
    return best_raw_cost, best_config, best_normalized_cost
...
# 阶段1: 同构全局优化
    ...
    print(f"最优损失: {homo_best_reward:.6f}")
    ...
    print(f"同构最优成本: {homo_best_reward:.6f} (将用于归一化强化学习奖励)")
    
    # 使用强化学习进行异构微调
    hetero_best_cost, hetero_best_config, hetero_normalized_cost = hetero_finetune_with_rl(
        homo_config, homo_best_reward, hetero_directory, hetero_max_rounds
    )
    ...
    print("\n" + "="*50)
    print(f"异构微调完成!")
    print(f"同构最优成本: {homo_best_reward:.6f}")
    print(f"异构最优成本: {hetero_best_cost:.6f}")
    print(f"归一化成本: {hetero_normalized_cost:.6f}")
    
    improvement = homo_best_reward - hetero_best_cost
    if hetero_best_cost < homo_best_reward:
        improvement_percentage = (homo_best_reward - hetero_best_cost) / homo_best_reward * 100
